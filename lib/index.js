...
//              hijack 'next' function
                    args[2] = function () {
                        ...
                        time.time = {
                            seconds:      now[0] - time.hr_start[0],
                            milliseconds: utils.get_ms_from_ns(now[1] - time.hr_start[1])
                        };
...
module.exports = function (app, settings) {
    ...
    var debug_render = function (view, options, fn) {
        ...
        if (settings.use_profiler) {
            ...
            req.EDT_req_time = {
                seconds:      now[0] - req.EDT_start[0],
                milliseconds: utils.get_ms_from_ns(now[1] - req.EDT_start[1])
            };

            var last_route = req.EDT_middleware_times[req.EDT_middleware_times.length - 1];
            if (!last_route.time) {
                last_route.time = {
                    seconds:      now[0] - last_route.hr_start[0],
                    milliseconds: utils.get_ms_from_ns(now[1] - last_route.hr_start[1])
                };
